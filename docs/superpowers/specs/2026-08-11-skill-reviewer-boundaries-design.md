...
suggestion 只保留一层无歧义的字面限制：出现 `grep`、`shell`、`parser`、`script`、`SQL`、`curl`、`外部工具`、`第三方工具`、`工具`、`命令`、`解析器`、`脚本`、`网络访问` 或 `网络请求` 等明确禁用字面量时直接拒绝，不判断该字面量处于推荐还是否定语境。需要表达“删除 grep”的模型输出可在 repair 时改写为“删除未授权能力”。这条规则只保证列出的字面量不会落库，不扩展成任意工具识别器。
...
聚焦单元测试锁定责任边界：混入多词英文正文、繁体文本和明确禁用字面量必须拒绝；孤立技术标识和安全 Evidence Policy 必须接受；未知 `awk` 语义不由 `parse_review()` 分类。System Prompt 测试继续确保能力、证据和停止条件要求不会从模型契约中消失。
