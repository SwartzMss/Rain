...
describe('issue skill runner', () => {
  ...
  it('runs an enabled private skill and shows its evidence', async () => {
    ...
    vi.mocked(rainApi.fetchSkillRunResult).mockResolvedValue({ summary: '发现数据库超时', observations: [], inferences: [], missing_context: [], evidence: [{ id: 'e1', bundle_hash: 'bundle-a', file_id: 8, path: '/logs/app.log', start_line: 42, end_line: 43, excerpt: 'timeout', explanation: '超时证据' }] });
    const reveal = vi.fn();
