...
export function SkillEditor({ skill, saving, onSave, onCancel }: { skill?: UserSkill | null; saving: boolean; onSave: (payload: SkillPayload) => Promise<void>; onCancel: () => void }) {
  ...
  return (
    <form className="space-y-4 rounded-xl border border-slate-200 bg-slate-50 p-4" onSubmit={submit}>
      ...
      <div className="flex gap-2"><button className="rounded-lg bg-slate-950 px-4 py-2 text-sm font-semibold text-white disabled:opacity-50" disabled={saving} type="submit">保存</button><button className="rounded-lg border border-slate-300 px-4 py-2 text-sm" type="button" onClick={onCancel}>取消</button></div>
    </form>
  );
}
