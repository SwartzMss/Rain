    ...
    <section className="space-y-4">
      ...
      {editing !== undefined ? <SkillEditor skill={editing} saving={busy} onSave={save} onCancel={() => setEditing(undefined)} /> : (
        <div className="grid gap-4 lg:grid-cols-[280px_1fr]">
          ...
          {selectedSkill ? <div className="space-y-4 rounded-xl border border-slate-200 p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="text-lg font-semibold">{selectedSkill.name}</h3><p className="text-sm text-slate-500">{selectedSkill.description || '无描述'}</p></div><div className="flex gap-2"><button className="rounded border px-3 py-1.5 text-sm" onClick={() => setEditing(selectedSkill)}>编辑</button><button className="rounded border px-3 py-1.5 text-sm" disabled={busy} onClick={() => void mutate(() => rainApi.updateSkill(selectedSkill.id, { name: selectedSkill.name, description: selectedSkill.description, skill_markdown: selectedSkill.skill_markdown, enabled: !selectedSkill.enabled }))}>{selectedSkill.enabled ? '停用' : '启用'}</button><button className="rounded border border-rose-200 px-3 py-1.5 text-sm text-rose-700" onClick={() => { if (window.confirm('确认删除该 Skill？')) void mutate(() => rainApi.deleteSkill(selectedSkill.id)); }}>删除</button></div></div><pre className="max-h-80 overflow-auto whitespace-pre-wrap rounded-lg bg-slate-950 p-4 text-sm text-slate-100">{selectedSkill.skill_markdown}</pre>{aiProviderConfigured === false ? <p className="text-sm text-amber-700">尚未配置 AI 模型服务，暂时无法进行质量评估。</p> : null}<button className="rounded-lg bg-cyan-600 px-4 py-2 text-sm font-semibold text-white disabled:opacity-50" disabled={busy || aiProviderConfigured !== true} onClick={() => void mutate(() => rainApi.reviewSkill(selectedSkill.id))}>质量评估</button><SkillReviewPanel review={selectedSkill.review} /></div> : null}
        </div>
      )}
    </section>
  );
