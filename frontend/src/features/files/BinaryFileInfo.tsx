    ...
    <section className="flex min-h-0 flex-1 flex-col" aria-label="二进制文件信息">
      <div className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-800 pb-5">
        <div className="flex min-w-0 items-center gap-3">
          <span className="flex h-10 w-12 shrink-0 items-center justify-center rounded border border-amber-500/40 bg-amber-500/10 text-xs font-semibold text-amber-200">
            BIN
          </span>
          <div className="min-w-0">
            <h2 className="break-all text-base font-semibold text-white">{node.name}</h2>
      ...
      <dl className="mt-5 divide-y divide-slate-800 border-y border-slate-800 text-sm">
        <div className="grid grid-cols-[120px_minmax(0,1fr)] gap-4 py-3">
          ...
          <dd className="break-all text-slate-200">{node.name}</dd>
        </div>
        <div className="grid grid-cols-[120px_minmax(0,1fr)] gap-4 py-3">
          <dt className="text-slate-500">文件类型</dt>
          <dd className="break-all text-slate-200">{node.mime_type || 'application/octet-stream'}</dd>
        </div>
        <div className="grid grid-cols-[120px_minmax(0,1fr)] gap-4 py-3">
          <dt className="text-slate-500">文件大小</dt>
          <dd className="text-slate-200">{formatSize(node.size_bytes)}</dd>
        </div>
      </dl>
    </section>
  );
