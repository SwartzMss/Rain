...
export function IssueSkillRunner({ issueCode, onRevealEvidence }: { issueCode: string; onRevealEvidence: (evidence: SkillEvidence) => void }) {
  ...
  useEffect(() => {
    Promise.all([rainApi.fetchSkills(), rainApi.fetchAiProviderStatus()]).then(([items, provider]) => {
      ...
      setSkills(enabled); setSelected(enabled[0]?.id ?? ''); setProviderReady(provider.configured);
    }).catch((reason) => setLoadError(normalizeApiError(reason)));
  }, [issueCode]);
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      ...
      {!providerReady && !loadError ? <p className="mt-3 text-sm text-amber-700">管理员尚未配置 AI Provider。</p> : null}
      {state.run ? <p className="mt-3 text-xs text-slate-500">状态：{state.run.status} · 迭代 {state.run.iteration_count}/8 · 工具调用 {state.run.tool_call_count}/24</p> : null}
