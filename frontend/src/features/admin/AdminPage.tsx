...
export function AdminPage() {
  ...
  }, [auth.state, query, status, userHistory, auditHistory]);
  ...
  const act = async (action: () => Promise<unknown>, message: string) => {
    ...
    setError(null);
    try {
      await runAdminAction({ action, reload: load, refreshAuth: auth.refresh });
      setNotice(message);
    } catch (actionError) {
      ...
      <section className="panel space-y-3">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead><tr><th>用户</th><th>状态</th><th>Session</th><th>操作</th></tr></thead>
            <tbody>{users.map((user) => <tr key={user.id}><td>{user.username}</td><td>{user.status}</td><td>{user.active_session_count}</td><td className="space-x-2"><button onClick={() => void act(() => rainApi.changeUserStatus(user.id, user.status === 'ACTIVE' ? 'DISABLED' : 'ACTIVE'), '状态已更新')}>{user.status === 'ACTIVE' ? '停用' : '启用'}</button><button onClick={() => void act(() => rainApi.revokeUserSessions(user.id), 'Session 已撤销')}>强制注销</button></td></tr>)}</tbody>
          </table>
        </div>
