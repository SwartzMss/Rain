...
export function AdminSettingsPage() {
  ...
  const save = async (value?: boolean, thresholds = false) => { setSaving(true); setMessage(null); setSaveError(null); try { const result = thresholds ? await rainApi.updateAdminSettings(undefined, ipLimit, usernameLimit) : await rainApi.updateAdminSettings(value); setAllowed(result.allow_registration); setIpLimit(result.login_ip_limit_per_minute); setUsernameLimit(result.login_username_failure_limit_per_5_minutes); setMessage('设置已保存'); } catch (e) { setSaveError(normalizeApiError(e)); await load(); } finally { setSaving(false); } };
  const saveThresholds = () => void save(undefined, true);
  return <AdminGuard><section className="max-w-3xl rounded-2xl border border-slate-200 bg-white/95 p-6 shadow-xl"><h1 className="text-xl font-semibold text-slate-950">系统设置</h1><div className="mt-6 flex items-center justify-between rounded-xl border border-slate-200 p-4"><div><h2 className="font-semibold">用户注册</h2><p className="mt-1 text-sm text-slate-500">关闭后，新用户将无法注册；已有用户仍可正常登录和使用系统。</p></div><button type="button" disabled={loading || saving || !hasLoadedSettings || Boolean(loadError)} onClick={() => void save(!allowed)} className={`relative h-7 w-12 rounded-full transition ${allowed ? 'bg-cyan-600' : 'bg-slate-300'} disabled:opacity-50`}><span className={`absolute top-1 h-5 w-5 rounded-full bg-white shadow transition ${allowed ? 'left-6' : 'left-1'}`} /></button></div><div className="mt-4 grid gap-3 sm:grid-cols-2"><label className="text-sm text-slate-600">IP 每分钟阈值<input type="number" min="1" max="1000" value={ipLimit} disabled={loading || saving || !hasLoadedSettings} onChange={(e) => setIpLimit(Number(e.target.value))} className="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2" /></label><label className="text-sm text-slate-600">用户名失败 5 分钟阈值<input type="number" min="1" max="100" value={usernameLimit} disabled={loading || saving || !hasLoadedSettings} onChange={(e) => setUsernameLimit(Number(e.target.value))} className="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2" /></label></div><button type="button" disabled={loading || saving || !hasLoadedSettings || Boolean(loadError)} onClick={saveThresholds} className="mt-4 rounded-lg bg-slate-950 px-4 py-2 text-sm font-semibold text-white disabled:opacity-50">保存限流配置</button>{message ? <p className="mt-4 text-sm text-emerald-700">{message}</p> : null}{loadError ? <p className="mt-4 text-sm text-rose-700">{loadError}</p> : null}{saveError ? <p className="mt-4 text-sm text-rose-700">保存失败：{saveError}</p> : null}</section></AdminGuard>;
}
