...
export function AdminSettingsPage() {
  ...
  const load = useCallback(async () => { setLoading(true); setLoadError(null); try { const value = await rainApi.fetchAdminSettings(); setAllowed(value.allow_registration); setHasLoadedSettings(true); } catch (e) { setHasLoadedSettings(false); setLoadError(normalizeApiError(e)); } finally { setLoading(false); } }, []);
