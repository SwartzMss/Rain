...
use crate::{
    ...
    search::{
        SearchIndex, SearchTimeWindow, SkillSearchMode, SkillSearchRequest,
    ...
    pub async fn search_logs_with_expansion(
        ...
        let result = SqliteFtsSearchIndex::new(self.state.db.pool.clone())
            .search_skill(SkillSearchRequest {
                ...
                mode: match search_mode { SearchMode::Fts => SkillSearchMode::Fts, SearchMode::ShortLiteral => SkillSearchMode::ShortLiteral },
                path_prefix, bundle_hash, file_id,
                time_window: applied_scope.as_ref().map(|scope| SearchTimeWindow { start_key: scope.start_ms, end_key: scope.end_ms }),
                fetch_limit: max_hits.saturating_add(1),
            }).await?;
        let has_unindexed_matches = result.has_unindexed_matches;
