...
pub async fn load_or_initialize_system_settings(
    ...
    sqlx::query("INSERT OR IGNORE INTO system_settings(id, allow_registration, login_ip_limit_per_minute, login_username_failure_limit_per_5_minutes, issue_inactive_days, adaptive_modes_json) VALUES(1, ?, ?, ?, ?, ?)")
        .bind(allow_registration as i64).bind(ip).bind(username).bind(issue_inactive_days)
        .bind(serde_json::json!({
            "upload_concurrent_processing_tasks": "auto",
            "search_tantivy_max_writers": "auto",
            "search_tantivy_writer_heap_size": "auto"
        }).to_string())
