...
async fn runner_repairs_a_structured_result_with_forged_evidence() {
    ...
    let (cancellation, _) = state.skill_runs.register(&run.id);
    let forged = r#"{"summary":"forged","observations":[],"inferences":[],"missing_context":[],"evidence":[{"file_id":999,"path":"/other.log","start_line":1,"end_line":2,"excerpt":"x","explanation":"x"}]}"#;
