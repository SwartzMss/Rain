...
- `skill_runs` 保存发起用户、Issue、Skill 身份、版本和临时正文快照，以及状态、计数器、取消标记、最终 JSON 和净化后的错误。可选的 `analysis_start_time`/`analysis_end_time` 保存规范化 UTC 时间文本，内部毫秒边界用于恢复 Runner 的不可变分析范围。范围最大 24 小时；未设置范围的 Run 保持兼容。`skill_id` 不设外键，因此删除源 Skill 不会破坏已开始的任务；用户和 Issue 删除会级联删除任务。
- `search_logs` 的 `context_expansion_minutes` 只能在 `0..=15` 内扩展已保存的主窗口；它不能替换 Run 范围，也不能让没有标准化事件时间的 chunk 进入有范围搜索。
